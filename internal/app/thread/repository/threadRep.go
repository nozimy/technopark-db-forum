...
func (t ThreadRepository) Vote(thread *model.Thread, vote *model.Vote) (*model.Thread, error) {
	...
	stmt, err := tx.Prepare("UPDATE votes SET voice = $3 WHERE nickname = $1 AND thread=$2")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()

	stmtThread, err := tx.Prepare("UPDATE threads SET votes = votes+$2 WHERE id=$1 RETURNING slug, title, message, forum, author, created, votes, id")
	if err != nil {
		return nil, err
	}
	defer stmtThread.Close()

	row := tx.QueryRow("SELECT voice FROM votes WHERE nickname = $1 AND thread=$2", vote.Nickname, thread.ID)
	var prevVoice int32
	...
	if vote.Voice != prevVoice {
		rowT := stmtThread.QueryRow(thread.ID, vote.Voice)
		err = rowT.Scan(
			&thread.Slug,
			&thread.Title,
			&thread.Message,
...
func (t ThreadRepository) GetThreadPosts(thread *model.Thread, limit, desc, since, sort string) (model.Posts, error) {
	...
	if sort == "flat" {
		...
		query += " ORDER BY path[1] " + desc + ", path ASC LIMIT " + limit
	...
	for rows.Next() {
		...
		if sort == "parent_tree" {
			childRows, err := t.db.Query("SELECT id, parent, thread, forum, author, created, message, isedited, path FROM posts WHERE thread = $1 AND path && ARRAY[$2::bigint]", thread.ID, p.ID)
			if err != nil {
				return nil, err
			}
...
func (t ThreadRepository) FindByIdOrSlug(id int, slug string) (*model.Thread, error) {
	...
	err := t.db.QueryRow(
		"SELECT slug, title, message, forum, author, created, votes, id FROM threads WHERE id=$1 OR slug=$2",
		id,
		slug,
	).Scan(
		&th.Slug,
